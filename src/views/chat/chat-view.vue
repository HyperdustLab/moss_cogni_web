...
const handleStopReasoning = () => {
  ...
  if (isProcessing.value || sendLoading.value) {
    ...
    // Remove only the AI response message, keep user input
    if (messageList.value.length >= 1) {
      const lastMessage = messageList.value[messageList.value.length - 1]

      // Only remove if the last message is an AI response
      if (lastMessage.type === 'ASSISTANT') {
        messageList.value.pop() // Remove only the last AI message
      }
    }
